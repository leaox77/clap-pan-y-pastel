...
export default function Reportes() {
  ...
  async function cargarDatos() {
    ...
    try {
      ...
      let items = []
      if (ventaIds.length) {
        const { data, error } = await supabase.from('venta_items').select('venta_id,cantidad,subtotal,costo_unitario_snapshot,productos(nombre)').in('venta_id', ventaIds)
      ...
      const topProd = {}
      items.forEach(i => { const n = i.productos?.nombre ?? 'Desconocido'; if (!topProd[n]) topProd[n] = { cantidad: 0, total: 0, costo: 0 }; topProd[n].cantidad += Number(i.cantidad || 0); topProd[n].total += Number(i.subtotal || 0); topProd[n].costo += Number(i.costo_unitario_snapshot || 0) * Number(i.cantidad || 0) })
      ...
      setDatos({ totalVentas, totalTx: ventas.length, costo, totalGastos, utilidadBruta, utilidadNeta, porMedio, topSorted, porHora, porDia, mermasTotales: mermas.reduce((s, m) => s + Math.abs(Number(m.cantidad || 0)), 0), ultimasVentas: ventas.slice(0, 15).map(v => ({ ...v, vendedor: perfilesMap[v.usuario_id] ?? '—', sucursal: sucursalesMap[v.sucursal_id] ?? '—' })), reservasEntregadas: reservas.filter(r => r.estado === 'entregada').length, reservasPendientes: reservas.filter(r => r.estado === 'pendiente').length, reservasCanceladas: reservas.filter(r => r.estado === 'cancelada').length })
  ...
  return (
    <div className="page-wrap">
      ...
      {datos && !loading && <>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 12, marginBottom: 20 }} className="grid-4">{[['Ventas',`Bs ${datos.totalVentas.toFixed(2)}`],['Transacciones',datos.totalTx],['Costo',`Bs ${datos.costo.toFixed(2)}`],['Utilidad bruta',`Bs ${datos.utilidadBruta.toFixed(2)}`],['Gastos',`Bs ${datos.totalGastos.toFixed(2)}`],['Utilidad neta',`Bs ${datos.utilidadNeta.toFixed(2)}`]].map(([l,v],i) => <div key={l} className="card" style={{ padding: '12px 16px' }}><p style={{ fontSize: 10, color: 'var(--text-soft)', textTransform: 'uppercase', marginBottom: 4 }}>{l}</p><p style={{ fontSize: 18, fontWeight: 700, color: l.includes('Utilidad') ? (l === 'Utilidad bruta' ? (datos.utilidadBruta >= 0 ? 'var(--ok)' : 'var(--err)') : (datos.utilidadNeta >= 0 ? 'var(--ok)' : 'var(--err)')) : 'var(--text)' }}>{v}</p></div>)}</div>
        ...
        <div className="card" style={{ overflow: 'hidden', marginBottom: 20 }}><div style={{ padding: '14px 20px', borderBottom: '1px solid var(--silver-light)', fontWeight: 700 }}>Productos más vendidos</div><div className="table-scroll"><table className="clap-table"><thead><tr><th>#</th><th>Producto</th><th>Unidades</th><th>Total vendido</th><th>Costo</th><th>Margen</th></tr></thead><tbody>{datos.topSorted.length === 0 ? <tr><td colSpan={6} style={{ textAlign: 'center', color: 'var(--text-soft)', padding: 24 }}>Sin datos</td></tr> : datos.topSorted.map(([nombre,d],idx) => <tr key={nombre}><td>{idx + 1}</td><td style={{ fontWeight: 600 }}>{nombre}</td><td>{d.cantidad}</td><td style={{ fontWeight: 700 }}>Bs {d.total.toFixed(2)}</td><td>Bs {d.costo.toFixed(2)}</td><td style={{ fontWeight: 700, color: d.total - d.costo >= 0 ? 'var(--ok)' : 'var(--err)' }}>Bs {(d.total - d.costo).toFixed(2)}</td></tr>)}</tbody></table></div></div>
        <div className="card" style={{ overflow: 'hidden' }}><div style={{ padding: '14px 20px', borderBottom: '1px solid var(--silver-light)', fontWeight: 700 }}>Últimas ventas</div><div className="table-scroll"><table className="clap-table"><thead><tr><th>Hora</th><th>Total</th><th>Medio</th><th>Vendedor</th><th>Sucursal</th></tr></thead><tbody>{datos.ultimasVentas.length === 0 ? <tr><td colSpan={5} style={{ textAlign: 'center', color: 'var(--text-soft)', padding: 24 }}>Sin datos</td></tr> : datos.ultimasVentas.map(v => <tr key={v.id}><td style={{ color: 'var(--text-soft)', whiteSpace: 'nowrap' }}>{new Date(v.fecha).toLocaleTimeString('es-BO', { hour: '2-digit', minute: '2-digit' })}</td><td style={{ fontWeight: 700 }}>Bs {Number(v.total).toFixed(2)}</td><td><span className="badge-info">{v.medio_pago}</span></td><td style={{ color: 'var(--text-soft)', fontSize: 12 }}>{v.vendedor}</td><td style={{ fontSize: 12 }}>{v.sucursal}</td></tr>)}</tbody></table></div></div>
      </>}
    </div>
  )
}
