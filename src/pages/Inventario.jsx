...
export default function Inventario() {
  ...
  const [fMerma, setFMerma] = useState({
    tipo: 'merma_danado',
    cantidad: '',
    nota: '',
  })
  ...
  async function fetchData() {
    ...
    if (errorProds) {
      toast(errorProds.message, 'err')
      return
    }

    if (errorCats) {
      toast(errorCats.message, 'err')
      return
    }
     ...
     * }
     *
     * El componente necesita trabajar directamente
     * con las propiedades del producto.
     *
     * Por eso aquí normalizamos la respuesta.
     */
    ...
    const productosNormalizados = (prods ?? [])
      ...
      .map((row) => {
        ...
        return {
          ...
          // ID de la fila de inventario de la sucursal
          ...
          // Categoría ya accesible si se necesita
          categoria_nombre:
  ...
  async function guardarEdicion() {
    ...
    const stockAnterior = Number(modalEditar.stock_actual)
    ...
    if (!fEditar.nombre?.trim()) {
      toast('El nombre es obligatorio', 'err')
      return
    }

    if (
      fEditar.precio_venta === '' ||
      Number(fEditar.precio_venta) < 0
    ) {
      toast('El precio de venta no es válido', 'err')
      return
    }

    if (stockNuevo !== stockAnterior) {
      console.log('Cambio de stock:', {
        producto: modalEditar.nombre,
    ...
    const { error } = await supabase.rpc('editar_producto', {
      p_id: modalEditar.producto_id ?? modalEditar.id,
  ...
  async function crearProducto() {
    const nombre = fProd.nombre.trim()
    const precio = Number(fProd.precio_venta)
    ...
    if (Number.isNaN(precio) || precio < 0) {
      toast('El precio no es válido', 'err')
      return
    }
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        stock_minimo: Number(fProd.stock_minimo || 0),
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {Object.entries(MERMA_LABELS).map(
            ([k, v]) => (
      ...
      >
        ...
        >
          ⚠ Los cambios quedan registrados en el
          log de auditoría con tu usuario y hora.
        </div>
